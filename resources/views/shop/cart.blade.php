...
<div class="container mx-auto p-4">
    <div class="flex justify-between items-center mb-5">
        {{-- We maken de titel dynamisch voor de duidelijkheid --}}
        <h1 class="text-3xl font-bold">Winkelmandje {{ ucfirst($category) }} 🛒</h1>
        
        {{-- AANPASSING 1: Categorie meegeven aan de terug-knop --}}
        <a href="{{ route('shop.index', $category) }}" class="px-4 py-2 bg-gray-500 text-white no-underline rounded hover:bg-gray-600 transition">
            ← Terug naar bestellen
        </a>
    </div>

    @if(session('cart_' . $category)) {{-- Check specifiek dit mandje --}}
        <div class="overflow-x-auto">
            <table class="w-full border-collapse border border-gray-300">
                ...
                <tbody>
                    {{-- Loop door de sessie van DEZE categorie --}}
                    @foreach(session('cart_' . $category) as $id => $details)
      ...
      {{-- AANPASSING 2: Categorie meegeven aan de checkout actie --}}
      <form id="checkout-form" action="{{ route('shop.checkout', $category) }}" method="POST" class="space-y-4">
        @csrf
    ...
    @else
        <p class="text-gray-600">Je {{ $category }} winkelmandje is nog leeg.</p>
        
        {{-- AANPASSING 3: Categorie meegeven aan de lege-mand-terug-knop --}}
        <a href="{{ route('shop.index', $category) }}" class="text-blue-600 hover:text-blue-800">Terug naar de shop</a>
    @endif
</div>
